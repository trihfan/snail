...
int main(int, char*[])
{
    ...
    mesh_v1->addVertex(vector3<double>(0.5, 0.25, -0.25));
    mesh_v1->addTriangle(0, 1, 2);

    auto mesh_v2 = std::make_unique<mesh<double>>(1);
    mesh_v2->addVertex(vector3<double>(-0.25, -0.25, -0.25));
    mesh_v2->addVertex(vector3<double>(0.5, 0.5, -0.25));
    mesh_v2->addVertex(vector3<double>(0.5, 0.25, -0.25));
    mesh_v2->addTriangle(0, 1, 2);

    log::info << "Pre compute - vertices: " << mesh_1->getVertexCount() << ", triangles: " << mesh_1->getTriangleCount() << std::endl;

    // Difference
    //mesh_1->sub(mesh_2->clone());
    mesh_1->sub(mesh_3->clone());

    log::info << "Post compute - vertices: " << mesh_1->getVertexCount() << ", triangles: " << mesh_1->getTriangleCount() << std::endl;
    log::err.clear();
